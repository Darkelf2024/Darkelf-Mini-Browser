...
from PySide6.QtGui import QPalette, QColor, QKeySequence, QShortcut, QAction, QGuiApplication, QActionGroup, QIcon, QPixmap, QPainter, QFont, QCursor, QPainterPath, QKeyEvent
...
from PySide6.QtCore import QUrl, QSettings, Qt, QObject, Slot, QTimer, QCoreApplication, Signal, QThread, QSize, QPoint, QByteArray, QEvent
...
class SuperHardenedPage(QWebEnginePage, _UAChNavigatorHardMixin, _DarkelfLetterboxMixin, CanvasProtectionMixin):
    ...
    def inject_navigator_darkelf_baseline(self):
        ...
        (() => {
          ...
          const defs = {
            userAgent: "Mozilla/5.0 (Windows NT 10.0; rv:115.0) Gecko/20100101 Firefox/78.0",
            platform: "Win32",
            vendor: "",
            language: "en-US",
...
def make_shield_icon(hex_color: str, size: int) -> QIcon:
    ...
    path.quadTo(cx, h, w * 0.15, h * 0.35)  # rounded bottom
    path.closeSubpath()

    pen = p.pen()
    pen.setColor(color)
    pen.setWidth(max(2, int(size * 0.10)))
    p.setPen(pen)
    p.setBrush(Qt.NoBrush)
    p.drawPath(path)

    # Lock keyhole (circle + rectangle)
    p.setBrush(color)
    r = max(2, int(size * 0.18))
    p.drawEllipse(int(cx - r/2), int(h*0.40), r, r)
    p.drawRect(int(cx - r/6), int(h*0.40 + r), int(r/3), int(r*0.9))
...
class Darkelf(QMainWindow):
    ...
    def eventFilter(self, obj, ev):
        """Work around macOS Caps Lock 'first key ignored/late' quirk.
        Only touches a–z immediately after a Caps Lock toggle, otherwise no-op.
        """
        try:
            if ev.type() == QEvent.KeyPress:
                ...
                # Detect Caps Lock toggle keystroke itself (some platforms deliver a key code; on others only the state changes).
                if key in (Qt.Key_CapsLock,):  # if delivered as a real key event
                    # Toggle our internal idea of caps state immediately
                    self._caps_on = not getattr(self, "_caps_on", False)
                    self._caps_toggled_at = time.monotonic()
                    # Let the event continue to OS/Qt
                    return False

                # If a letter key arrives, fix just-after-toggle lowercase issue
                text = ev.text() or ""
                if text and ("a" <= text <= "z"):
                    # If OS didn’t give us a CapsLock keypress event, try to infer: on macOS,
                    # the first letter after toggle may come too soon; we only intervene briefly.
                    toggled_recently = (time.monotonic() - getattr(self, "_caps_toggled_at", 0)) < 0.30

                    if toggled_recently:
                        # In Latin layouts, uppercase = Caps XOR Shift
                        want_upper = bool(getattr(self, "_caps_on", False)) ^ bool(mods & Qt.ShiftModifier)

                        if want_upper and text.islower():
                            # Re-emit the same key as uppercase, then swallow the original.
                            ...
                            if target is not None:
                                new_ev = QKeyEvent(
                                    QEvent.KeyPress,
                                    key,
                                    mods | Qt.ShiftModifier,   # nudge Qt into uppercase path
                                    text.upper()
                                )
                                QCoreApplication.postEvent(target, new_ev)
                                return True  # swallow original lowercase press
            elif ev.type() == QEvent.KeyRelease and ev.key() in (Qt.Key_CapsLock,):
                # Some platforms report the toggle on release only.
                self._caps_on = not getattr(self, "_caps_on", False)
                self._caps_toggled_at = time.monotonic()
                return False
            ...
            # Fail safe: never block input if anything goes wrong
            return False
    ...
    def init_shortcuts(self):
        """Cross-platform shortcuts using StandardKey + app context."""
        def bind(stdkey, handler):
            sc = QShortcut(QKeySequence(stdkey), self)
            sc.setContext(Qt.ApplicationShortcut)      # works anywhere in the window
        ...
        # Reload/Refresh
        bind(QKeySequence.StandardKey.Refresh,  self.reload_page)    # F5 / Cmd+R

        # Zoom
        bind(QKeySequence.StandardKey.ZoomIn,   self.zoom_in)        # Ctrl/Cmd + "+"
        bind(QKeySequence.StandardKey.ZoomOut,  self.zoom_out)       # Ctrl/Cmd + "-"

        # Tabs & window
        bind(QKeySequence.StandardKey.AddTab,   self.create_new_tab) # Ctrl/Cmd + T
        bind(QKeySequence.StandardKey.Close,    lambda: self.close_tab(self.tab_widget.currentIndex()))  # Ctrl/Cmd + W

        # Fullscreen (Qt maps to F11 on Win/Linux, Ctrl+Cmd+F on macOS)
        bind(QKeySequence.StandardKey.FullScreen, self.toggle_full_screen)

        # History (no standard key; keep a conventional one per OS)
        hist_seq = QKeySequence("Ctrl+H") if sys.platform != "darwin" else QKeySequence("Meta+Y")  # Safari uses Cmd+Y for history
        sc_hist = QShortcut(hist_seq, self); sc_hist.setContext(Qt.ApplicationShortcut); sc_hist.activated.connect(self.view_history)
    ...
    def create_toolbar(self):
        ...
        back_button    = self._btn(self.go_back, make_text_icon('◄', THEME['text'], nav_icon_size), "Back")
        forward_button = self._btn(self.go_forward, make_text_icon('►', THEME['text'], nav_icon_size), "Forward")
        reload_button  = self._btn(self.reload_page, make_text_icon('↺', THEME['text'], nav_icon_size), "Reload")
        ...
        zoom_in_button  = self._btn(self.zoom_in,  make_text_icon('+', THEME['text'], nav_icon_size), "Zoom In")
        zoom_out_button = self._btn(self.zoom_out, make_text_icon('−', THEME['text'], nav_icon_size), "Zoom Out")
        full_button     = self._btn(self.toggle_full_screen, make_text_icon('⛶', THEME['text'], nav_icon_size), "Full Screen")
        
        # Protection indicator (starts enabled/green)
        self._protect_button = self._btn(
            # action when clicked (optional: open a security/about panel)
            getattr(self, "open_security_panel", lambda: None),
            make_shield_icon("#22c55e", nav_icon_size),   # green
            "Darkelf Protection: Enabled"
        )

        toolbar.addWidget(self._protect_button); toolbar.addSeparator()
...
"""
    ...
    def show_context_menu(self, pos):
        ...
        if data and hasattr(data, "linkUrl") and data.linkUrl().isValid():
            ...
            menu.addSeparator()
        # Edit actions (triggerAction always for copy and paste)
    ...
    def closeEvent(self, event):
        try:
            # Wipe all browser data on exit
            self.web_profile.clearHttpCache()
            self.web_profile.cookieStore().deleteAllCookies()
            self.web_profile.clearAllVisitedLinks()
            self.history_log.clear()
            self.tab_histories.clear()

            # Clear clipboard on exit for extra privacy
            QGuiApplication.clipboard().clear()

            # Optionally, redirect all tabs to the homepage before closing (visually wipes before shutdown)
