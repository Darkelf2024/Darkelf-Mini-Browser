...
from PySide6.QtGui import QPalette, QColor, QKeySequence, QShortcut, QAction, QGuiApplication, QActionGroup, QIcon, QPixmap, QPainter, QFont, QCursor, QPainterPath, QKeyEvent, QTextCursor, QPen, QBrush, QLinearGradient
...
class Darkelf(QMainWindow):
    def __init__(self, start_url: str = "home"):
        ...
        self.init_ui()
        self.init_shortcuts()
        self._setup_clipboard_auto_wipe()
                
        # First tab: homepage or provided URL
        self.create_new_tab(start_url)
        
    def _setup_clipboard_auto_wipe(self, delay_ms=10000):
        clipboard = QGuiApplication.clipboard()
        def auto_wipe():
            QTimer.singleShot(delay_ms, lambda: clipboard.clear(mode=clipboard.Clipboard))
        clipboard.dataChanged.connect(auto_wipe)
    ...
    def create_toolbar(self):
        ...
        new_id_button = self._btn(
            ...
            "New Identity (Clear storage & reload tab)"
        )
        ...
        toolbar.addWidget(reload_button); toolbar.addWidget(new_id_button)   # <-- Add new identity here
        toolbar.addSeparator(); toolbar.addWidget(home_button)
...
"""
    ...
    def new_identity_for_current_tab(self):
        reply = QMessageBox.question(
            self, "New Identity",
            "This will clear all cookies/storage for this tab, request a new Tor circuit, and reload. Continue?",
