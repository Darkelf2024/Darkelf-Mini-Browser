...
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QTabWidget, QPushButton, QLineEdit, QVBoxLayout, QMenuBar, QToolBar, QDialog, QMessageBox, QFileDialog, QProgressDialog, QListWidget, QMenu, QWidget, QLabel, QToolButton, QSizePolicy, QFrame, QHBoxLayout, QTextEdit, QGraphicsDropShadowEffect, QWidget, QPlainTextEdit, QWidgetAction
)
...
def get_or_create_canvas_seed():
    path = os.path.expanduser("~/.darkelf_pyside6_canvas_seed.txt")
...
class IPLeakGuard(QWebEngineUrlRequestInterceptor):
    """
    Block localhost & RFC1918/private/link-local hosts. Only inspect the host.
    """
    def interceptRequest(self, info):  # type: ignore[override]
        ...
        # Block local schemes explicitly if you want:
        if scheme in ("file", "ftp"):
            info.block(True)
            print(f"[IPLeakGuard] Blocked protocol: {scheme}://")
            return

        # Empty or no host → nothing to do
        if not host:
            return
        ...
        if host in ("localhost",) or host.endswith(".local"):
            info.block(True)
            print(f"[IPLeakGuard] Blocked localhost: {host}")
            return

        # If host is an IP, use ipaddress to classify
        try:
            ip = ipaddress.ip_address(host)
            if ip.is_private or ip.is_loopback or ip.is_link_local:
                info.block(True)
                print(f"[IPLeakGuard] Blocked private IP: {host}")
                return
        except ValueError:
            # Not an IP (hostname) → optional: block common LAN suffixes
            if host.endswith((".lan", ".home", ".intranet")):
                info.block(True)
                print(f"[IPLeakGuard] Blocked LAN hostname: {host}")
                return
...
# --- Darkelf inline ADBLOCK_JS ---
ADBLOCK_JS = r"""
(function(){
if(window.__darkelf_adblock_injected)return;window.__darkelf_adblock_injected=true;
let trackersBlocked=0;
const trackerPatterns=[
/doubleclick\.net/i,/adservice\.google\.com/i,/googlesyndication\.com/i,/google-analytics\.com/i,/googletagmanager\.com/i,/facebook\.net/i,/facebook\.com.*\/tr\?/i,/pixel.*\.io/i,/adsafeprotected\.com/i,/hotjar\.com/i,/mixpanel\.com/i,/segment\.com/i,/taboola\.com/i,/outbrain\.com/i,/ads-twitter\.com/i,/scorecardresearch\.com/i,/1x1(\.png|\.gif|\.jpg)/i,/\/pixel(\.gif|\.png|\.jpg)/i,/\/track(\.gif|\.png|\.jpg)/i,/adroll\.com/i,/adnxs\.com/i,/ads\.yahoo\.com/i,/criteo\.com/i,/pubmatic\.com/i,/rubiconproject\.com/i,/openx\.net/i,/yieldmo\.com/i,/mathtag\.com/i,/quantserve\.com/i,/scorecardresearch\.com/i
];
function updateBadge(){
try{let e=document.getElementById('trackers-badge');if(e){e.textContent=trackersBlocked;e.style.display=trackersBlocked>0?'inline-block':'none';}}catch(_){}
}
...
if(name==='src'&&value){for(let patt of trackerPatterns){if(patt.test(value)){trackersBlocked++;updateBadge();return;}}}
...
let el=origCreateElement.call(document,tag);
if(tag.toLowerCase()==='img'||tag.toLowerCase()==='iframe'||tag.toLowerCase()==='script'){
...
if(tag.toLowerCase()==='iframe'){setTimeout(function(){if((el.width<=1&&el.height<=1)||el.style&&(parseInt(el.style.width)<=1&&parseInt(el.style.height)<=1)){el.remove();trackersBlocked++;updateBadge();}},100);}
return el;
};
document.addEventListener('DOMContentLoaded',function(){
let imgs=Array.from(document.images);
for(let img of imgs){
if((img.width<=1&&img.height<=1)||trackerPatterns.some(patt=>patt.test(img.src))){img.remove();trackersBlocked++;}
}
let iframes=Array.from(document.querySelectorAll('iframe'));
for(let f of iframes){
if((f.width<=1&&f.height<=1)||trackerPatterns.some(patt=>patt.test(f.src))){f.remove();trackersBlocked++;}
}
let scripts=Array.from(document.scripts);
for(let s of scripts){
let src=s.src||"";for(let patt of trackerPatterns){if(patt.test(src)){s.remove();trackersBlocked++;}}
}
updateBadge();
});
let origFetch=window.fetch;
window.fetch=function(){
let url=arguments[0];if(typeof url==='string'&&trackerPatterns.some(patt=>patt.test(url))){trackersBlocked++;updateBadge();return new Promise(()=>{});}
return origFetch.apply(this,arguments);
};
let origXHROpen=XMLHttpRequest.prototype.open;
XMLHttpRequest.prototype.open=function(method,url){
if(typeof url==='string'&&trackerPatterns.some(patt=>patt.test(url))){trackersBlocked++;updateBadge();return;}
return origXHROpen.apply(this,arguments);
};
// YouTube ad removal
if(location.hostname.includes('youtube.com')){
setInterval(()=>{document.querySelectorAll('.ad-showing,ytd-ad-slot-renderer,.ytp-ad-module').forEach(e=>e.remove());let skip=document.querySelector('.ytp-ad-skip-button');if(skip)skip.click();},500);
}
})();
"""
...
class _UAChNavigatorHardMixin:
    def inject_uach_off_everywhere(self):
        ...
        const UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:78.0) Gecko/20100101 Firefox/78.0";
...
class YoutubeMixin:
    ...
    - Applies minimal canvas "protection": canvas API is fully available, but returns a unique, stable value per user (via seeded output).
    - No canvas randomization or noise; instead, any fingerprint test will show "100% unique" due to the fixed seed.
    - Blocks cookies, disables local storage, disables plugins.
    - Hooks mitmproxy adblocker (if present).
    - Drop-in: call YoutubeMixin().apply_to_page(page, url, tor_active) after navigation if 'youtube.com' in URL.
    """

    def __init__(self, adblocker=None):
        self.adblocker = adblocker

    def apply_to_page(self, page, url=None, tor_active=False):
        if not url:
            try:
        ...
        # Always inject seed-canvas: canvas is available, but fingerprint is always unique per user/seed
        self.inject_canvas_seed(page)
        self.spoof_chrome_user_agent(page)
    ...
    def inject_canvas_seed(self, page):
        # Use the same seed logic as global: unique per install, persistent
        try:
            seed = get_or_create_canvas_seed()
        except Exception:
            # fallback to environment or default
            import os
            seed = os.environ.get("DARKELF_CANVAS_SEED", "darkelf-default-seed")
        js = ("""
(function(seedString){
function xfnv1a(str) {
    var h = 2166136261>>>0;
...
var seed = (seedString||"darkelf-default-seed");
var num = xfnv1a(seed);
var rng = mulberry32(num);
var _cachedW=64,_cachedH=64;
function generateImageData(w,h){
    var img = new ImageData(w,h);
    var d = img.data;
    for(var y=0;y<h;y++)for(var x=0;x<w;x++){
    var i=(y*w+x)*4;
    var v1 = Math.floor(64+64*Math.sin((x+num%37)*0.11+rng()*6.283));
    var v2 = Math.floor(64+64*Math.cos((y+num%53)*0.09+rng()*6.283));
    var v3 = Math.floor(96+96*(rng()*0.5+0.25));
    d[i+0]=(v1+v3)&0xFF;
    d[i+1]=(v2+v3)&0xFF;
    d[i+2]=(v1^v2)&0xFF; d[i+3]=255;
    } return img;
}
var _cachedImg = generateImageData(_cachedW,_cachedH);
// Patch only the 64x64 at 0,0 read for "uniqueness" tests; all others normal
CanvasRenderingContext2D.prototype.getImageData = function(x,y,w,h){
    if(w===_cachedW&&h===_cachedH&&x===0&&y===0) return _cachedImg;
    // Use normal getImageData for all other reads
    return Object.getPrototypeOf(CanvasRenderingContext2D.prototype).getImageData.call(this, x, y, w, h);
};
HTMLCanvasElement.prototype.toDataURL = function(type){
    var c = document.createElement('canvas');
    c.width=_cachedW;c.height=_cachedH;
    var ctx=c.getContext('2d');
    ctx.putImageData(_cachedImg,0,0);
...
HTMLCanvasElement.prototype.toBlob = function(cb,type,q){
    ...
    var ctx=c.getContext('2d');
    ctx.putImageData(_cachedImg,0,0);
    return c.toBlob(cb,type||'image/png',q);
};
})("%SEED%");
""").replace("%SEED%", seed)
        self.inject_script(page, js, injection_point=None)  # Default: DocumentCreation

    def spoof_chrome_user_agent(self, page):
        js = """
    (() => {
        // Spoof Chrome UA (Win10, Chrome 120)
        const ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.6099.224 Safari/537.36";
        Object.defineProperty(navigator, 'userAgent', { get: () => ua, configurable: true });
        Object.defineProperty(navigator, 'appVersion', { get: () => ua, configurable: true });
        Object.defineProperty(navigator, 'platform', { get: () => "Win32", configurable: true });
        // Optionally spoof sec-ch-ua headers too
        const fakeHeaders = {
            'sec-ch-ua': '"Chromium";v="120", "Not:A-Brand";v="99"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
        };
        // Patch fetch/XHR to always send these
        const origFetch = window.fetch;
        window.fetch = function(resource, init = {}) {
            init.headers = new Headers(init.headers || {});
            for (const h in fakeHeaders) { try { init.headers.set(h, fakeHeaders[h]); } catch(e){} }
            return origFetch(resource, init);
        };
        const origOpen = XMLHttpRequest.prototype.open;
        XMLHttpRequest.prototype.open = function(...args) {
            this.addEventListener("readystatechange", function() {
                if (this.readyState === 1) {
                    for (const h in fakeHeaders) { try { this.setRequestHeader(h, fakeHeaders[h]); } catch(e){} }
                }
            });
            return origOpen.apply(this, args);
        };
    ...
    def inject_script(self, page, script_str, injection_point=None):
        from PySide6.QtWebEngineCore import QWebEngineScript
        script = QWebEngineScript()
        script.setSourceCode(script_str)
        # Always use DocumentCreation for earliest injection (no DocumentStart in PySide6)
        script.setInjectionPoint(QWebEngineScript.DocumentCreation if injection_point is None else injection_point)
        script.setWorldId(QWebEngineScript.MainWorld)
...
class DarkelfCompatSandbox:
    ...
    def inject_extra_spoofing(self, page):
        ...
        ua_script = """
        (() => {
            Object.defineProperty(navigator, 'userAgent', { get: () => "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:78.0) Gecko/20100101 Firefox/78.0" });
...
class SuperHardenedPage(QWebEnginePage, _UAChNavigatorHardMixin, _DarkelfLetterboxMixin, CanvasProtectionMixin):
    ...
    def acceptNavigationRequest(self, url, _type, isMainFrame):
        ...
        # Block fingerprinting test sites when JS is disabled
        if hasattr(self, "view") and self.view:
            try:
                js_enabled = self.view.settings().testAttribute(QWebEngineSettings.JavascriptEnabled)
            except Exception:
                js_enabled = True
        else:
            js_enabled = True
        ...
        test_sites = [
            #"amiunique.org", "browserleaks.com", "fpcentral", "pixelscan.net", "coveryourtracks.eff.org", "fingerprint.com", "creepjs"
        ]
        if not js_enabled and any(site in url.toString().lower() for site in test_sites):
            QTimer.singleShot(0, lambda: self.view.setHtml("""
                ...
                <body style="background:#111;color:#fff;font-family:sans-serif;text-align:center;padding-top:60px;">
                    ...
                    <p>Tracking/fingerprinting has been blocked: JavaScript is disabled for your protection.<br>
                    No unique browser or OS data is exposed.</p>
                </body>
                </html>
            """, url))
            return False
    ...
    def inject_adblock_js(self):
        """Inject the inline JS adblocker at DocumentStart (subframes too)."""
        try:
            self.inject_script(ADBLOCK_JS, injection_point=QWebEngineScript.DocumentStart, subframes=True)
        except Exception:
            try:
                self.inject_script(ADBLOCK_JS, injection_point=None, subframes=True)
            except Exception:
                pass
    ...
    def inject_navigator_prototype_spoof(self):
        # final surface values (Firefox 78 on Win32; vendor empty)
        script = r"""
        (() => {
          const UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:78.0) Gecko/20100101 Firefox/78.0";
...
class Darkelf(QMainWindow):
    def __init__(self, start_url: str = "home"):
        ...
        self._ipguard = IPLeakGuard(self)
        DarkelfHardenedProfile.install(self.web_profile, remove_iframes=True)
        self._composite = CompositeInterceptor(
            self._adblock, self._strip, self._ipguard, parent=self
        )
        self.web_profile.setUrlRequestInterceptor(self._composite)

        # --- UI must come first (so toolbar exists) ---
        self.init_ui()

        # --- Now add adblock badge and connect ---
        self._ensure_adblock_badge()
        self._adblock.blocked.connect(self._on_blocked)
    ...
    def _update_adblock_badge_from_js(self):
        v = self.current_view()
        if not v:
            return
        # Also support fallback for pages where window.trackersBlocked is not defined
        js = "(window.trackersBlocked !== undefined ? window.trackersBlocked : (window.__darkelf_adblock_injected ? window.trackersBlocked : 0));"
        def cb(val):
            try:
                if hasattr(self, "_adblock_badge"):
                    self._adblock_badge.setText(str(int(val) if val is not None else 0))
            except Exception:
                pass
                
        v.page().runJavaScript(js, cb)
    ...
    def toggle_tor_profile(self):
        self.tor_network_enabled = not self.tor_network_enabled
        self.settings.setValue("tor_network_enabled", self.tor_network_enabled)

        # Recreate profile and re-apply hardening/interceptors
        self.web_profile = self.build_web_profile()
        DarkelfHardenedProfile.install(self.web_profile, remove_iframes=True)
        self.web_profile.setUrlRequestInterceptor(self._composite)
        ...
        if self.tor_network_enabled:
            self.init_tor()
            self._protect_button.setIcon(make_shield_icon("#22c55e", 24))
            self._protect_button.setToolTip("Darkelf Protection: Enabled (Tor Mode)")
            self.reload_all_tabs(tor_mode=True)
        else:
            self.stop_tor()
            self.disable_tor_proxy()
            self._protect_button.setIcon(make_shield_icon("#b7b7b7", 24))
            self._protect_button.setToolTip("Darkelf Protection: Minimal (YouTube Mode)")
            self.reload_all_tabs(tor_mode=False)
            
    def disable_tor_proxy(self):
        # Remove QNetworkProxy
        QNetworkProxy.setApplicationProxy(QNetworkProxy())
        # Remove proxy from env (optional but safest)
        flags = os.environ.get("QTWEBENGINE_CHROMIUM_FLAGS", "")
        flags = re.sub(r'--proxy-server="[^"]*"\s*', '', flags)
    ...
    def _ensure_adblock_badge(self):
        """
        Creates (once) a small pill badge in the toolbar to show the number of blocked requests.
        Safe to call multiple times.
        """
        # Prefer the main toolbar you already built
        tb = getattr(self, "toolbar", None)

        # If not set yet, try to find any existing toolbar in the window
        if tb is None:
            tbs = self.findChildren(QToolBar)
            tb = tbs[0] if tbs else None

        # If still none, bail out (do NOT create a new toolbar)
        if tb is None:
            return

        if getattr(self, "_adblock_badge_action", None) is not None:
            return

        badge = QLabel("0", parent=tb)
        badge.setStyleSheet("QLabel{background:#222;color:#a7ff7a;border-radius:12px;padding:0 8px;font-weight:600;}")
        badge.setFixedHeight(24)

        act = QWidgetAction(tb)
        act.setDefaultWidget(badge)
        tb.addAction(act)

        self._adblock_badge = badge
        self._adblock_badge_action = act
        
    def _on_blocked(self, url: str):
        """Slot called when AdblockInterceptor blocks a URL."""
        try:
            if hasattr(self, "_adblock_badge"):
                current = int(self._adblock_badge.text())
                self._adblock_badge.setText(str(current + 1))
        except Exception:
            pass
...
"""
    ...
    def create_new_tab(self, url="home"):
        ...
        if self.tor_network_enabled:
            ...
            page = QWebEnginePage(self.web_profile, view)
            # JS adblocker: inject as soon as possible
            script = QWebEngineScript()
            ...
            # --- Inject PyQt5-style seeded canvas globally (unique per install) ---
            seed = get_or_create_canvas_seed()
            canvas_js = """
(function(seedString){
  function xfnv1a(str) {
...
var img = new ImageData(w,h);
var d = img.data;
for(var y=0;yh;y++)for(var x=0;x<w;x++){
  var i=(y*w+x)*4;
  var v1 = Math.floor(64+64*Math.sin((x+num%37)*0.11+rng()*6.283));
  var v2 = Math.floor(64+64*Math.cos((y+num%53)*0.09+rng()*6.283));
  var v3 = Math.floor(96+96*(rng()*0.5+0.25));
...
""".replace("%SEED%", seed)
            canvas_script = QWebEngineScript()
            canvas_script.setSourceCode(canvas_js)
            canvas_script.setInjectionPoint(QWebEngineScript.DocumentCreation)
            canvas_script.setWorldId(QWebEngineScript.MainWorld)
            canvas_script.setRunsOnSubFrames(True)
            page.profile().scripts().insert(canvas_script)

            # No YoutubeMixin needed; canvas script applies to all sites
    ...
    def reload_all_tabs(self, tor_mode: bool):
        self.web_profile = self.build_web_profile()
        # Remove all tabs and recreate them as new views/pages with correct hardening
        urls = []
        for i in range(self.tab_widget.count()):
            old_view = self.tab_widget.widget(i)
            url = old_view.url().toString() if hasattr(old_view, "url") else "home"
            urls.append(url)
        ...
        for url in urls:
            ...
            if tor_mode:
                ...
                page = QWebEnginePage(self.web_profile, view)
                # JS adblocker always in minimal mode
                script = QWebEngineScript()
                script.setSourceCode(ADBLOCK_JS)
                script.setInjectionPoint(QWebEngineScript.DocumentCreation)
                script.setWorldId(QWebEngineScript.MainWorld)
                script.setRunsOnSubFrames(True)
                page.profile().scripts().insert(script)
                YoutubeMixin(self._adblock).apply_to_page(page, url, tor_active=False)
            view.setPage(page)
            view.settings().setAttribute(QWebEngineSettings.JavascriptEnabled, self.javascript_enabled)
            view.loadFinished.connect(self._update_tab_title)
            view.loadFinished.connect(self._update_adblock_badge_from_js)
            ...
            view.customContextMenuRequested.connect(self.show_context_menu)
            if url == "home":
                view.setHtml(self.custom_homepage_html(), QUrl("about:blank"))
            else:
                view.setUrl(QUrl(url))
            self.tab_widget.addTab(view, "Reloaded")
            view.show()
