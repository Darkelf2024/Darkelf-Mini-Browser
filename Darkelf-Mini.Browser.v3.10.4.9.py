...
"""
Darkelf Mini — PySide6 (Monolithic Build)

Features
--------
• Robust UA-CH stripping (no sec-ch-ua* leaks) via QWebEngineUrlRequestInterceptor
• SuperHardenedPage with full shield pack (runs at DocumentCreation)
• Consolidated QTWEBENGINE_CHROMIUM_FLAGS (no duplicates)
• Tor support:
   - Env-based Chromium proxy (DARKELF_TOR=1 or DARKELF_PROXY=... at launch)
   - Optional local Tor launcher using stem
   - QNetworkProxy fallback wiring
     (note: Chromium primarily honors --proxy-server)
...
---------------------
Defends against:
- Persistent disk artifacts (cache, cookies, storage)
- Casual to moderate browser fingerprinting
- Accidental data persistence across sessions

Best-effort only:
- In-process memory overwrites (Python limitations apply)

Out of Scope:
- Kernel-level attackers
- Live memory capture / debugging
- Compromised OS or Python interpreter
- Hardware forensic attacks

Design Notes
------------
- This file is intentionally monolithic.
- Structure is provided via phases, headers, and scoped helpers.
- Behavior correctness takes precedence over refactor purity.
