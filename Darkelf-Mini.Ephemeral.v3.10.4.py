...
class SuperHardenedPage(QWebEnginePage, _UAChNavigatorHardMixin, _DarkelfLetterboxMixin, CanvasProtectionMixin):
    ...
    def inject_navigator_prototype_spoof(self):
        # final surface values (Firefox 128 on Linux; vendor empty)
        script = r"""
        (() => {
          const UA = "Mozilla/5.0 (X11; Linux x86_64; rv:128.0) Gecko/20100101 Firefox/128.0";
...
class TorCircuitDialog(QDialog):
    def __init__(self, circuit_info, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Tor Circuit")
        self.setMinimumSize(480, 240)
        layout = QVBoxLayout(self)
        title = QLabel("<b>Current Tor Circuit for this site</b>")
        layout.addWidget(title)
        if circuit_info:
            for hop in circuit_info:
                label = QLabel(f"{hop['country']} ({hop['ip']}) <i>{hop['nickname']}</i>")
                layout.addWidget(label)
        else:
            layout.addWidget(QLabel("<i>Could not determine circuit for this site.</i>"))
...
class TorCircuitInfoBox(QFrame):
    ...
    def set_circuit_info(self, hops):
        ...
        while self.layout.count() > 1:
            ...
            if w: w.deleteLater()
        if not hops:
            self.layout.addWidget(QLabel("<i>Could not determine circuit for this site.</i>"))
        else:
            for hop in hops:
                label = QLabel(f"{hop['country']} ({hop['ip']}) <i>{hop['nickname']}</i>")
...
class Darkelf(QMainWindow):
    ...
    def create_toolbar(self):
        ...
        toolbar.addWidget(new_id_button)
    ...
    def on_mask_button_clicked(self):
        # Toggle infobox: show if hidden, hide if visible
        ...
        if not self._circuit_infobox:
            self._circuit_infobox = TorCircuitInfoBox(self)
